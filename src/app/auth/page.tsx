...
const ConfigureProtocol = () => {
  ...
  const [message, setMessage] = useState<{ isSuccess: boolean; text: string | null }>({
    isSuccess: false,
    text: null,
  });
  ...
  const handleQueryProtocol = async () => {
    try {
      ...
      if (protocols[0]) {
        setQueryProtocolStatus('Installed protocol: ' + JSON.stringify(protocols[0].definition.protocol));
      } else {
